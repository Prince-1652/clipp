...
function cellAt(ch: string, row: number, col: number, phase: Phase, t: number, delay: number, theme: Theme) {
  if (ch === ' ') return {ch, color: theme.primary, dim: false}
  const baseColor = ch === '▓' ? '#c3c2c4' : theme.primary
  if (phase === 'idle') return {ch, color: baseColor, dim: false}
  if (phase === 'intro') {
    const dt = t - delay
    ...
    return {ch, color: baseColor, dim: false}
  }
  ...
  if (d <= HALF && 1 - d / HALF > 0.35) {
    ...
    return {ch: LIGHTER[ch] ?? ch, color: baseColor, dim: false}
  }
  return {ch, color: baseColor, dim: false}
}
