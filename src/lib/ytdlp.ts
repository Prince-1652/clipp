...
const CLIPP_DIR = path.join(os.homedir(), '.clipp', 'bin')
...
export async function ensureYtDlp(onStatus: (message: string) => void, signal?: AbortSignal): Promise<string> {
  const local = path.join(CLIPP_DIR, process.platform === 'win32' ? 'yt-dlp.exe' : 'yt-dlp')
  ...
  if (stat) {
    ...
    if (ageDays > 7) {
      ...
      await fs.utimes(local, new Date(), new Date()).catch(() => {})
    }
    return local
  }

  onStatus('first run: fetching yt-dlp…')
  await fs.mkdir(CLIPP_DIR, {recursive: true})
...
export async function probe(ytdlp: string, url: string, signal?: AbortSignal): Promise<ProbeResult> {
  ...
  const infoJsonPath = path.join(os.tmpdir(), `clipp-info-${process.pid}-${Date.now()}.json`)
...
export function buildChoices(info: VideoInfo): DownloadChoice[] {
  ...
  const videoGroups = new Map<string, RawFormat>()
  for (const v of videos) {
    const fps = v.fps ? (v.fps > 30 ? Math.round(v.fps).toString() : '') : ''
    const key = `${v.height}p${fps}-${v.ext}`
    if (!videoGroups.has(key)) {
      videoGroups.set(key, v)
    } else {
      const existing = videoGroups.get(key)!
      if ((v.tbr ?? 0) > (existing.tbr ?? 0)) {
        videoGroups.set(key, v)
      }
    }
  }

  const sortedVideos = Array.from(videoGroups.values()).sort((a, b) => {
    if (a.height !== b.height) return (b.height ?? 0) - (a.height ?? 0)
    if (a.fps !== b.fps) return (b.fps ?? 0) - (a.fps ?? 0)
    return (b.tbr ?? 0) - (a.tbr ?? 0)
  })

  for (const v of sortedVideos.slice(0, MAX_VIDEO_CHOICES * 2)) {
    const muxed = v.acodec && v.acodec !== 'none'
    const size = (v.filesize ?? v.filesize_approx ?? 0) + (muxed ? 0 : audioSize ?? 0)
    const sizeLabel = size > 0 ? ` · ~${formatBytes(size)}` : ''
    const fps = v.fps ? (v.fps > 30 ? Math.round(v.fps).toString() : '') : ''
    choices.push({
      kind: 'video',
      label: `${v.height}p${fps} · ${v.ext}${sizeLabel}`,
      args: ['-f', muxed ? v.format_id : `${v.format_id}+ba/b`, '--merge-output-format', v.ext === 'mp4' ? 'mp4' : 'mkv'],
    })
  }
  ...
  const audioGroups = new Map<string, RawFormat>()
  for (const a of audioOnly) {
    const key = a.ext ?? 'unknown'
    if (!audioGroups.has(key)) {
      audioGroups.set(key, a)
    } else {
      const existing = audioGroups.get(key)!
      if ((a.abr ?? a.tbr ?? 0) > (existing.abr ?? existing.tbr ?? 0)) {
        audioGroups.set(key, a)
      }
    }
  }
  const sortedAudio = Array.from(audioGroups.values()).sort((a, b) => (b.abr ?? b.tbr ?? 0) - (a.abr ?? a.tbr ?? 0))

  for (const a of sortedAudio) {
    const size = a.filesize ?? a.filesize_approx
    const sizeLabel = size ? ` · ~${formatBytes(size)}` : ''
    choices.push({
      kind: 'audio',
      label: `audio only · ${a.ext}${sizeLabel}`,
      args: ['-f', a.format_id],
    })
  }

  if (!sortedAudio.some(a => a.ext === 'mp3')) {
    const sizeLabel = audioSize ? ` · ~${formatBytes(audioSize)}` : ''
    choices.push({
      kind: 'audio',
      label: `audio only · mp3${sizeLabel}`,
